...
public class Project1 {
	...
	public static void main(String[] args) throws FileNotFoundException {
		...
		ArrayList<Matrix> class_points = new ArrayList<Matrix>();
		ArrayList<Matrix> b_point = new ArrayList<Matrix>();
		for (double i = -4; i<5; i+=0.1) {
			for (double j = -6; j<10; j+=0.1) {
				Matrix points = new Matrix (new double[][] {{i},{j}}); 
				class_points.add(points);
			}
		}
		
		
		for (int i=0; i<class_points.size();i++) {
			Matrix p1c1 = a1.find_discriminant(class_points.get(i),mean1,inverse1,deter1);
			Matrix p1c2 = a1.find_discriminant(class_points.get(i),mean2,inverse2,deter2);
			a1.boundary_plot(class_points.get(i), p1c1, p1c2, b_point);		
		}
		
		/*System.out.println(b_point.size());
		for (int i=0; i<b_point.size();i++) {
			System.out.println(b_point.get(i).getValue(1, 0));	
		}*/	
		...
		//10. Find condition number
		System.out.println("Condition number: "+linear.find_condition(linear_inverse));
